...
class Render_Puml__Jira_Graph:
    ...
    def fix_card_text(self, card_text):
        if card_text:
            return card_text.replace('\n', "\\n")       # make sure there are no 'new lines' in the card text (since that will break plantUml)
        return card_text
    ...
    def on_add_node(self, element, card_text, id_plant_uml, id_jira):
        ...
        if issue:
            ...
            card_text       = self.fix_card_text(card_text)
    ...
    def resolve_card_color(self, issue):
        ...
        if issue_type == 'FACT'                 : return 'LightBlue'
        if issue_type == 'TIMELINE'             : return 'Cornsilk'
        if issue_type == 'MITRE ATT&CK'         : return 'HoneyDew'
        if issue_type == 'THREAT ACTOR'         : return 'MistyRose'
        if issue_type == 'IP ADDRESS'           : return 'LightGoldenRodYellow'
        if issue_type == 'EXPLOIT'              : return 'LightPink'
        if issue_type == 'CUSTOMER'             : return 'MediumAquaMarine'
        if issue_type == 'IP DATA'              : return 'Ivory'
